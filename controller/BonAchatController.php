...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($op) && !empty($op)) {
        ...
        if ($op == "add") {
            if (!$ps->create(new BonAchat(NULL,$fournisseur,$date,$etat)))
                $response = false;
        }else if ($op == "update") {
            if (!$ps->update(new BonAchat($id,$fournisseur,$date,$etat)))
                $response = false;
        }else if ($op == "updateEtat") {
            if (!$ps->updateEtat(new BonAchat($id,"","",$etat)))
                $response = false;
        }else if ($op == "delete") {
            if (!$ps->delete($id))
                $response = false;
            ...
            $data = $ps->getLastPrice($bon,$produit);
        }else if ($op == "lastID") {
            $data = $ps->LastID();
