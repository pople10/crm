...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($op) && !empty($op)) {
        ...
        if ($op == "add") {
            if (!$ps->create(new RetourAchat(NULL,$fournisseur,$date_creation,"",$etat)))
                $response = false;
        }else if ($op == "update") {
            if (!$ps->update(new RetourAchat($id,$fournisseur,$date_creation,"",$etat)))
                $response = false;
        }else if ($op == "updateEtat") {
            if (!$ps->updateEtat(new RetourAchat($id,"","",date("Y-m-d"),$etat)))
                $response = false;
        }else if ($op == "delete") {
            if (!$ps->delete($id))
                $response = false;
            ...
            $data = $ps->getLastPrice($retour,$produit);
        }else if ($op == "datatable"){
            // DB table to use
            $table = "RetourAchatView";
