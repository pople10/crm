...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($op) && !empty($op)) {
        ...
        if ($op == "add") {
            if (!$ps->create(new DevisAchat(NULL,$fournisseur,$date,$type)))
                $response = false;
        }else if ($op == "update") {
            if (!$ps->update(new DevisAchat($id,$fournisseur,$date,$type)))
                $response = false;
        }else if ($op == "updateType") {
            if (!$ps->updateType(new DevisAchat($id,"","",$type)))
                $response = false;
        }else if ($op == "delete") {
            if (!$ps->delete($id))
                $response = false;
            ...
            $data = $ps->findById($id);
        }else if ($op == "DevisToCommande") {
            $data = $ps->DevisToCommande($devis);
        }else if ($op == "getLastPrice") {
            $data = $ps->getLastPrice($devis,$produit);
        }else if ($op == "lastID") {
            $data = $ps->LastID();
        }else if ($op == "datatable"){
            // DB table to use
            $table = "DevisAchatView";
