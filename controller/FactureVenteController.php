...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($op) && !empty($op)) {
        ...
        if ($op == "add") {
            if (!$ps->create(new FactureVente(NULL,$client,$date,$type,null)))
                $response = false;
        }else if ($op == "update") {
            if (!$ps->update(new FactureVente($id,$client,$date,$type,null)))
                $response = false;
        }else if ($op == "updateType") {
            if (!$ps->updateType(new FactureVente($id,"","",$type,null)))
                $response = false;
        }else if ($op == "delete") {
            if (!$ps->delete($id))
                $response = false;
