...
include_once 'service/AvoirAchatService.php';

$ps = new AvoirAchatService();
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($op) && !empty($op)) {
        ...
        if ($op == "add") {
            if (!$ps->create(new AvoirAchat(NULL,$fournisseur,$date,$type,null)))
                $response = false;
        }else if ($op == "update") {
            if (!$ps->update(new AvoirAchat($id,$fournisseur,$date,$type,null)))
                $response = false;
        }else if ($op == "updateType") {
            if (!$ps->updateType(new AvoirAchat($id,"","",$type,null)))
                $response = false;
        }else if ($op == "delete") {
            if (!$ps->delete($id))
                $response = false;
            ...
            $data = $ps->getLastPrice($avoir,$produit);
        }else if ($op == "datatable"){
            // DB table to use
            $table = "AvoirAchatView";
