...
include_once 'service/AvoirVenteService.php';

$ps = new AvoirVenteService();
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($op) && !empty($op)) {
        ...
        if ($op == "add") {
            if (!$ps->create(new AvoirVente(NULL,$client,$date,$type,null)))
                $response = false;
        }else if ($op == "update") {
            if (!$ps->update(new AvoirVente($id,$client,$date,$type,null)))
                $response = false;
        }else if ($op == "updateType") {
            if (!$ps->updateType(new AvoirVente($id,"","",$type,null)))
                $response = false;
        }else if ($op == "delete") {
            if (!$ps->delete($id))
                $response = false;
