...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($op) && !empty($op)) {
        ...
        if ($op == "add") {
            if (!$ps->create(new DevisVente(NULL,$client,$date,$type)))
                $response = false;
        }else if ($op == "update") {
            if (!$ps->update(new DevisVente($id,$client,$date,$type)))
                $response = false;
        }else if ($op == "updateType") {
            if (!$ps->updateType(new DevisVente($id,"","",$type)))
                $response = false;
        }else if ($op == "delete") {
            if (!$ps->delete($id))
                $response = false;
