...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($op) && !empty($op)) {
        ...
        if ($op == "add") {
            if (!$ps->create(new CommandeVente(NULL,$client,$date,"En cours")))
                $response = false;
        }else if ($op == "update") {
            if (!$ps->update(new CommandeVente($id,$client,$date,"En cours")))
                $response = false;
        }else if ($op == "updateEtat") {
            if (!$ps->updateEtat(new CommandeVente($id,"","",$etat)))
                $response = false;
        }else if ($op == "delete") {
            if (!$ps->delete($id))
                $response = false;
