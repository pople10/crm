...
var fillPaimentsTable = function(id,tableName) {
    var thead = "<th>N°</th><th>Date</th><th>Type</th><th>Numéro de chèque</th><th>Montant</th>";
    $("#paiment-table").find("tbody").empty();
    $("#paiment-table").find("thead").empty().append(thead);
    $(".exportsection").hide();
    paimentsDATA = [];
    $.ajax({
        url: "../../../controller/Paiment"+tableName+"VenteController.php",
        cache: false,
        type:'POST',
        ...
        success: function(result) {
            ...
            if (result.data.length > 0) {
                ...
                result.data.forEach(e => {
                    ...
                    t += "<tr><td>"+e.id+"</td><td>" + e.date + "</td>" + type + "<td>" + e.numero_cheque + "</td>" + montant + "</tr>";
                });
...
var generatePDF2 = function (filename,d){
    ...
    var header = function(data) {
        ...
        doc.text("Les paiments "+tname+" n° "+d.id+" de client "+d.client+" sont : ",40,h,'left');
...
$(document).ready(function(){
    ...
    table = $('#datatable-releves').DataTable({
        ...
        "ajax": {
            url: "../../../controller/ReglementsVenteController.php",
            cache: false,
            type:'POST',
            data:{
                op:'datatable',
        ...
        "columns": [
            ...
            },
            {
                "data": null,
                "render": function(data) {
                    return '<button data-id="' + data.id + '" data-table="'+data.tablename+'" data-client="'+data.nom+'" data-total="'+data.total_ttc+'"  data-reste="'+data.reste+'" data-toggle="modal" data-animation="bounce" data-target=".paiment-modal" type="button" class="btn btn-sm btn-success" style="float: none; margin: 5px;"><span class="fas fa-archive"></span> Détails</button>';
                }
            }
        ],
        "initComplete": function( settings, json ) {
            ...
            if( client_select.val() == "" || types_select.val() == "" || table.$("input[name='checkbox']").length==0){
                $('#maincheck').hide();
        ...
        "drawCallback": function( settings ) {
            ...
            if(getUrlParameter('q') && settings.aoData.length)
               {
                    //console.log($(settings.aoData[0].anCells[0]).find("input[type='checkbox']").is(':checked'));
                    $(settings.aoData[0].anCells[0]).find("input[type='checkbox']").click();
                    //console.log($(settings.aoData[0].anCells[0]).find("input[type='checkbox']").is(':checked'));
                    if($(settings.aoData[0].anCells[0]).find("input[type='checkbox']").is(':checked'))
                        $("#reglee_btn").click();
                }
        }
    });
    if(getUrlParameter('q'))
       {
           table.columns(1).search(getUrlParameter('q'),true,false).draw();
       }
    $('.paiment-modal').on('show.bs.modal', function(event) {
        var button = $(event.relatedTarget);
        var id = button.attr("data-id");
        var client = button.attr("data-client");
        ...
        /*tablePaiment = $('#paiment-table').DataTable({
            ...
            "ajax": {
                url: "../../../controller/Paiment"+tname+"VenteController.php",
                cache: false,
                dataSrc: 'data',
                type:'POST',
        ...
        $(".btn-pdf2").attr("data-client",client);
    ...
    $(".btn-pdf2").click(function() {
        var iframe = document.getElementById('pdfHolder');
        iframe.src = "";
        var d = {id:$(this).attr("data-id"),tablename: $(this).attr("data-table"),client:$(this).attr("data-client"),total:$(this).attr("data-total"),reste:$(this).attr("data-reste")};
